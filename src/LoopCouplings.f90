...
Contains
 ...
 Subroutine CoupScalarPhoton(m_H2, mW2, g_W, mf_u2, g_u, mf_d2, g_d, mf_l2    &
    & , g_l, mC2, g_C, m_Hp2, g_Hp, mSup2, g_su, mSdown2, g_sd, mSlept2, g_sl &
    & , coup, coupSM)
 !----------------------------------------------------------------------------
 ! calculates the lowest order coupling for the radiative decay of a scalar
 ! to two photons
 ...
 Implicit None
  ...
  !--------------------------------------
   Do i1=1,3
    coupSM = coupSM + A_onehalf(mH2p/ mf_l2(i1)) + (A_onehalf(mH2p/ mf_d2(i1)) &
                                + 4._dp * A_onehalf(mH2p/ mf_u2(i1)) ) / 3._dp
   End Do
  End If
  ...
  !--------------------------------------
   Do i1=1,3
    coup = coup + g_l(i1) * A_onehalf(mH2p/ mf_l2(i1))    &
         ...
         &        + 4._dp * g_u(i1) * A_onehalf(mH2p/ mf_u2(i1)) ) / 3._dp
   End Do
  !--------------------------------------
  ! chargino contributions
  !--------------------------------------
  ...
  Do i1=1,6
   coup = coup + g_sl(i1) * A_zero(mH2p/ mSlept2(i1))   &
        &      + ( g_sd(i1) * A_zero(mH2p/ mSdown2(i1)) &
